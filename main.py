...
class PTBModel(object):
    def __init__(self, CellType, is_training, config):
        ...
        if is_training and config.keep_prob < 1:
            lstm_cell = core_rnn_cell.DropoutWrapper(lstm_cell, output_keep_prob=config.keep_prob)
...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    for i in range(num_epochs):
        ...
        saver.save(sess, checkpoint_path + 'checkpoints')
    ...
    t1 = time.time()
    print("Total running time for {%s} network is {%d} minutes"%(model_name, (t1-t0)/60))