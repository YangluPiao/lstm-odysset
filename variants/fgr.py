...
class FGRLSTMCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            c_prev, y_prev, i_prev, f_prev, o_prev = tf.split(state,num_or_size_splits=5,axis=1)
            ...
            i_bar = tf.matmul(inputs, W_i) + tf.matmul(y_prev, R_i) + tf.multiply(c_prev, p_i) + b_i + tf.matmul(i_prev, R_ii) + tf.matmul(f_prev, R_fi) + tf.matmul(o_prev, R_oi)
            i = tf.sigmoid(i_bar)

            f_bar = tf.matmul(inputs, W_f) + tf.matmul(y_prev, R_f) + tf.multiply(c_prev, p_f) + b_f + tf.matmul(i_prev, R_if) + tf.matmul(f_prev, R_ff) + tf.matmul(o_prev, R_of)
            f = tf.sigmoid(f_bar)

            c = tf.multiply(i, z) + tf.multiply(f, c_prev)

            o_bar = tf.matmul(inputs, W_o) + tf.matmul(y_prev, R_o) + tf.multiply(c, p_o) + b_o + tf.matmul(i_prev, R_io) + tf.matmul(f_prev, R_fo) + tf.matmul(o_prev, R_oo)
            o = tf.sigmoid(o_bar)
